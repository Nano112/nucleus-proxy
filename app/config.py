...
class Settings(BaseSettings):
    ...
    nucleus_password: str = Field(
        default="changeme123", description="Nucleus password (override in env/secrets)"
    )
    nucleus_k8s_mode: bool = Field(
        default=False,
        description="Enable Kubernetes service discovery for Nucleus stack",
    )

    # Proxy Server Configuration
    proxy_bind: str = "0.0.0.0:8088"
    proxy_secret: str = Field(
        default="change-me-in-production-default-key",
        description="Secret for signing tokens (override in env/secrets)",
    )
    proxy_workers: int = Field(
        default=1, ge=1, le=32, description="Number of Sanic worker processes"
    )
    ...
    upload_commit_max_attempts: int = Field(
        ...
        ge=1,
        le=10,
        description="Retries when uploading assembled file to Nucleus",
    )
    ...
    request_timeouts: int = Field(
        default=600, ge=30, le=3600, description="Request/response timeout in seconds"
    )
    slow_request_threshold_ms: int = Field(
        default=1000, ge=100, description="Slow request threshold in milliseconds"
    )

    # Monitoring / Metrics (M6)
    metrics_retention_hours: int = Field(
        default=24, ge=1, le=168, description="Metrics retention period"
    )
    metrics_max_points: int = Field(
        default=1000, ge=100, le=10000, description="Max metric points per series"
    )
    ...
    background_sync_enabled: bool = Field(
        default=True, description="Enable automatic background sync"
    )
    background_sync_interval_minutes: int = Field(
        default=60, ge=5, description="Background sync interval"
    )
    background_cleanup_interval_minutes: int = Field(
        default=30, ge=5, description="Background cleanup interval"
    )

    # Testing Configuration
    proxy_base_url: str = "http://127.0.0.1:8088"
    run_nucleus_it: Optional[str] = Field(
        default="0", description="Enable Nucleus integration tests"
    )
