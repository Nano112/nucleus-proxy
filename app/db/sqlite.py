...
class Database:
    ...
    async def get_all_sessions(self) -> List[UploadSession]:
        """Get all upload sessions from the database."""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                SELECT id, token_id, user_id, path_dir, filename, size, sha256,
                       part_size, received_bytes, created_at, state, error, meta
            ...
            for row in rows:
                try:
                    sessions.append(UploadSession(
                        ...
                        sha256=row[6],
                        part_size=row[7],
                        received_bytes=row[8],
                        created_at=datetime.fromisoformat(row[9]),
                        state=UploadState(row[10]),
                        error=row[11],
    ...
    async def get_session_by_token(self, upload_token: str) -> Optional[UploadSession]:
        ...
        try:
            ...
            logger.error(f"Database error getting session by token {upload_token}: {e}")
            return None
    ...
    async def delete_session(self, session_id: str) -> bool:
        """Delete a session and its parts from the database."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                # Delete parts first (foreign key constraint)
                await db.execute("DELETE FROM upload_parts WHERE session_id = ?", (session_id,))
                # Delete session
