...
class NucleusClient:
            ...
            try:
                ...
                while True:
                    ...
                    if expected_id is not None and response.get("id") not in (None, expected_id):
                        ...
                        continue
                    if response.get("status") == "INVALID" and not response.get("error"):
                        logger.debug(
                            "Received interim INVALID status for %s; waiting for next frame",
                            expected_id,
                        )
    ...
    async def delete_path(self, path: str, is_folder: bool = None) -> Dict[str, Any]:
        ...
        if responses and len(responses) > 0:
            ...
            if item_status in ['OK', 'Done', 'DONE']:
                ...
                return {"status": "FOLDER_NOT_EMPTY", "error": f"Folder is not empty: {path}"}
            else:
                return {"status": item_status, "error": f"Delete failed with status: {item_status}"}
    ...
    async def move_file(self, source_path: str, dest_path: str, message: str = "Moved via Nucleus Proxy") -> Dict[str, Any]:
        ...
        if result.get('results') and len(result['results']) > 0:
            ...
            return {
                "status": "OK" if single_result.get('success') else single_result.get('status', 'ERROR'),
                "src": source_path,
                "dst": dest_path,
                "error": single_result.get('error') if not single_result.get('success') else None
            }
        return result
    ...
    async def copy_file(self, source_path: str, dest_path: str, checkpoint: Optional[int] = None, 
                        message: str = "Copied via Nucleus Proxy") -> Dict[str, Any]:
