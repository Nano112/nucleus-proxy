...
from app.routes.auth import ensure_path_permission, require_auth, get_current_user
...
async def generate_upload_url(request: Request) -> HTTPResponse:
    ...
    try:
        ...
        permission_error = ensure_path_permission(request, req.path_dir, "write")
        if permission_error:
...
async def generate_download_url(request: Request) -> HTTPResponse:
    ...
    try:
        ...
        permission_error = ensure_path_permission(request, req.file_path, "read")
        if permission_error:
            return permission_error
